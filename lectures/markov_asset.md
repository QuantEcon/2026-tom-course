...
(For a refresher on notation and theory for finite Markov chains see {doc}`this lecture <intermediate:finite_markov>`)
...
From the [Neumann series lemma](https://python.quantecon.org/linear_algebra.html#the-neumann-series-lemma) and Gelfand's formula, equation {eq}`vcumrn` has a unique solution when $\beta K$ has spectral radius strictly less than one.
...
Recalling that $P^i {\mathbb 1} = {\mathbb 1}$ for all $i$ and applying [Neumann's geometric series lemma](https://python.quantecon.org/linear_algebra.html#the-neumann-series-lemma), we are led to
